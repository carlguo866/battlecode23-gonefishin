...
public class Headquarter extends Unit {
    public static void run() throws GameActionException {
        ...
        MapLocation spawnLocation = rc.getLocation().add(SPAWN_DIR[rc.getRoundNum()]).add(SPAWN_DIR[rc.getRoundNum()]);
        if (turnCount <= 3) {
                for (int[] dir : BFS25) {
                    ...
                    if(rc.canBuildRobot(RobotType.CARRIER, location)
                        ...
                        Comm.setSpawnQ(rc.getID() / 2, location.x, location.y, PURPURSES[rc.getRoundNum()]);
                        break;
                    }
                }
        }
