...
public class Launcher extends Unit {
    ...
    private static final int MAX_ENEMY_CNT = 6;
    ...
    static void run () throws GameActionException {
        if (lastLoc == null) lastLoc = rc.getLocation();
        if (turnCount == 0) {
    ...
    static void sense() throws GameActionException {
        ...
        enemyLauncherCnt = 0;
        closeFriendsSize = 0;
        int closestFriendlyLauncherDis = 0;
...
//                        int dis = distanceIgnoringWall(rc.getLocation(), robot.getLocation());
//                    }
//                    int newDis = distanceIgnoringWall(rc.getLocation(), robot.getLocation());
//                    if (newDis < closestFriendlyLauncherDis) {
//                        closestFriendlyLauncherDis = newDis;
//                        closestFriendlyLauncher = robot;
//                    }
    ...
    static void macro() throws GameActionException {
        ...
        //grouping
//        if ((attackTarget == null || rc.getRoundNum() - lastLauncherAttackRound < 3) && closeFriendsSize < 3
//                && groupingAttempt < 10) {
//            if (closestFriendlyLauncher != null && distanceIgnoringWall(rc.getLocation(), closestFriendlyLauncher.location) > 2) {
...
//        }
        ...
        enemyHQLoc = Comm.enemyHQLocations[enemyHQID];
        Direction moveDir = lastLoc.directionTo(rc.getLocation());
        Direction backDir = moveDir.opposite();
        if (closeFriendsSize < 4) {
            for (int friend_i = friendlyLauncherCnt; --friend_i >= 0;) {
                Direction friendDir = friendlyLaunchers[friend_i].location.directionTo(rc.getLocation());
                if (distanceIgnoringWall(friendlyLaunchers[friend_i].location, rc.getLocation()) > 1 && (
                    friendDir == backDir ||
                    friendDir == backDir.rotateLeft() ||
                    friendDir == backDir.rotateRight() )) {
                    System.out.println("in front stop");
                    return;
                }
            }
        }
...
//        if (rc.isMovementReady() && closeFriendsSize < 4) {
//            if (groupingTarget != null
//                    && !rc.getLocation().isAdjacentTo(groupingTarget.location)){
//                indicator += "group,";
//                System.out.println("grouping1");
//                moveToward(groupingTarget.location);
//            } else if (rc.getRoundNum() - lastLauncherAttackRound < 5 && cachedGroupingTarget != null) {
//                System.out.println("grouping2");
//                moveToward(cachedGroupingTarget.location);
//            }
//        }
    }
